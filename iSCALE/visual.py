...
def get_spot_boundary_mask(shape, locs, radius):
    mask_patch = get_disk_mask(radius, boundary_width=radius//10)
    mask = np.zeros(shape).astype(bool)
    for i, j in locs:
        mask[i-radius:i+radius+1, j-radius:j+radius+1] = mask_patch
    return mask


def draw_spot_boundary(img, locs, radius, color):
    mask = get_spot_boundary_mask(img.shape[:2], locs, radius)
    img[mask] = color
...
def plot_spots(
    ...
    if boundary_color is not None:
        draw_spot_boundary(img, locs, radius, boundary_color)
