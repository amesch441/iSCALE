...
def get_data_smooth(prefix , radius, dist):
    ...
    embs, cnts, (embs_mean, embs_std), (cnts_min, cnts_max) = normalize(embs, cnts)
  ...
  # change xy coordinates to ij coordinates
    ...
    x_out_high = locs2[:,0] < math.ceil(radius)  
    x_out_low = locs2[:,0] > embs.shape[0]-math.ceil(radius) 

    y_out_high = locs2[:,1] < math.ceil(radius) 
    y_out_low = locs2[:,1] > embs.shape[1]-math.ceil(radius) 
...
def main():
    ...
    print('Total runtime:', int(time()-t_all))
